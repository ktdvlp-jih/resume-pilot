...
배포/CI 장애는 아니지만, 운영 가시성 강화를 위해 다음을 추가 반영:

1. AI 로그에 모델명 표시 (`model`)  
   - 운영자가 실제 사용 모델/우회 여부를 즉시 확인 가능
2. 공고분석 AI 로그 누락 보완  
   - `job_analysis` 경로도 `ai_usage_logs` 기록
3. AI 로그 조회 성능 보강  
   - projection + 최근 건수 제한으로 Admin `/ai-logs` 체감 개선

*보고서 끝 — 2026-07-08 업데이트 포함*
