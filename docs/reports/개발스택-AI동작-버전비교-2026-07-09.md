...
> **outdated (2026-08):** LLM 실패 시 rule-based 콘텐츠 폴백은 제거됨. 현행은 `docs/아키텍처.md` 및 코드 기준(실패 시 오류 반환).
...
[C] 공고분석 (Job Analysis)
Gateway -> Prompt(JOB_ANALYSIS) -> LLM -> Postprocess
(* LLM 실패 시 오류 반환 — rule fallback 없음)
(* RAG 보통 미사용)
```
...
[C] 공고분석
게이트웨이 -> 프롬프트(JOB_ANALYSIS) -> LLM -> 후처리
(* LLM 실패 시 오류 반환 — 규칙 기반 fallback 없음)
(* RAG는 일반적으로 사용하지 않음)
```
...
resume-api -> resume-ai(Gateway) -> prompt-service(render) -> LLM -> 위험 문장/사유 반환 -> ai_usage_logs
(* LLM 실패 시 오류 반환)

[4] 공고 분석 (Job Analysis)
resume-api -> resume-ai(Gateway)
           -> (TEXT/URL/PDF/IMAGE 전처리)
           -> prompt-service JOB_ANALYSIS
           -> LLM 추출
           -> 실패 시 오류 반환
...
sequenceDiagram
    ...
    alt 생성(cover letter generation)
        ...
        AI->>AI: 실패 시 오류 반환 (rule fallback 없음)
    end
...
- LLM 실패 시 **오류를 반환**한다 (rule-based 콘텐츠 폴백 없음)
...
- LLM 실패를 숨기지 않아 장애 원인을 조기에 파악 가능
...
- 배포 직후:
  - `/admin/ai-logs`에서 `job_analysis` 모델/방법(`+llm`) 확인 (rule 경로 없음)
