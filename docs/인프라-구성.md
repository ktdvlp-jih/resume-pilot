...
> **설치·배포:** [설치-가이드.md](설치-가이드.md#part-3-ubuntu-서버) · **실행:** [실행-가이드.md](실행-가이드.md)

프로덕션(Ubuntu Docker)은 **컨테이너 5개** + **호스트 nginx(TLS)** 로 동작합니다.  
브라우저·nginx는 **`APP_PORT`(기본 9180) 하나**만 보면 됩니다.
...
[인터넷 / 브라우저]
        │
        ├─ https://resume.ggury.com  ── 호스트 nginx (:443 → :9180)
...
│  Ubuntu 서버                                             │
│                                                         │
│  nginx (sites-available/resume.ggury.com) ──► localhost:9180               │
│                                                         │
...
**로컬 개발**은 Docker 5개가 아니라 **터미널**에서 서비스별 실행 — [실행-가이드.md §1](실행-가이드.md#1-로컬-개발-터미널).
...
같은 서버에 다른 Docker 스택이 있으면 **호스트 포트가 겹치지 않게** `.env`에서 조정 — [서버-공존-가이드.md](서버-공존-가이드.md).
...
AI 생성은 60초를 넘길 수 있으므로 `proxy_read_timeout` / `proxy_send_timeout`은 **300s** (`scripts/nginx-resume.ggury.com.conf`).
...
| nginx (resume.ggury.com) | Ubuntu 호스트, Docker 밖 |
...
배포: [설치-가이드.md §3](설치-가이드.md#part-3-ubuntu-서버) · `./scripts/resume-pilot.sh deploy`
...
| 문서 | 내용 |
|------|------|
| [아키텍처.md](아키텍처.md) | 서비스 책임·디렉터리 |
| [설치-가이드.md](설치-가이드.md) | 설치·배포 진입 |
| [실행-가이드.md](실행-가이드.md) | 로컬 개발·E2E |
| [리눅스-서버-배포-가이드.md](리눅스-서버-배포-가이드.md) | 배포 트러블슈팅 |
