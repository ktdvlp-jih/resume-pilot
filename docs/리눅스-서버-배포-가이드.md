...
**진입 문서:** [설치-가이드.md § Part 3](설치-가이드.md#part-3-ubuntu-서버)
...
# Edit .env: POSTGRES_PASSWORD, JWT_SECRET, LAN_HOST (LLM keys: Admin /admin/llm-settings)
...
**nginx origin:** `http://127.0.0.1:9180` (`https://resume.ggury.com`)

Co-located stacks: [서버-공존-가이드.md](서버-공존-가이드.md)
...
| 4 | AI E2E TC-AI-05 | `scripts/deploy-smoke-e2e-ai.sh` | Playwright `ai-flow.spec.ts` (Admin LLM 라우트 필요) |
...
| `deploy-smoke-e2e-ai.sh` | CI AI E2E (`ai-flow.spec.ts`, Admin LLM 라우트 필요) |
| `deploy-smoke.ps1` | Windows / 공개 URL 수동 검증 |

수동 배포: `./scripts/resume-pilot.sh deploy` (스모크는 CI에서만 자동).

현황: [프로젝트-현황.md](프로젝트-현황.md) · 장애: [reports/배포-CI-장애-보고-2026-07-07.md](reports/배포-CI-장애-보고-2026-07-07.md)
...
| `INTERNAL_API_TOKEN` | 내부 서비스 토큰 |
| `LLM_ENCRYPTION_KEY` | Admin LLM 키 암호화 |
| LLM 모델·키 | Admin `/admin/llm-settings` (외부 AI API). `OPENAI_*`는 폴백 전용 |
...
See [설치-가이드.md](설치-가이드.md) and [실행-가이드.md](실행-가이드.md).
