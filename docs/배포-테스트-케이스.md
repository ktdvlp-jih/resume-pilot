...
> 마지막 업데이트: 2026-07-08 (LLM 라우팅·AI 로그 모델 표시 반영)  
...
| 환경 | URL |
|------|-----|
| LAN | `http://<LAN_HOST>:9180` |
| 공개 HTTPS | `https://resume.ggury.com` |

**AI 사전 조건**

| 항목 | 설명 |
|------|------|
| `INTERNAL_API_TOKEN` | 서버 `.env` 필수 — resume-ai/rag-service 내부 라우트 인증 |
| `LLM_ENCRYPTION_KEY` | 서버 `.env` 필수 — Admin 저장 API key 암호화 |
| Admin `/admin/llm-settings` | DeepSeek/Gemini 등 Provider 키 입력·활성화 (외부 API, 로컬 LLM 아님) |
| `EMBEDDING_DIMENSION` | 서버 `.env` 유지 (기본 1536, DB vector와 일치) |
...
# LLM 테스트 호출 (prompt-service 폴백 `OPENAI_*` 필요. 운영 생성은 Admin llm-settings)
...
| 2 | `POST /analyze/writing-style` | 휴리스틱(비LLM) | `frequent_words`, `tone`, `sentence_style` |
| 3 | `POST /generate/resume` | **rag** → **prompt** → LLM | `content`, `quality_scores`, `detections`, `reviews`, `experience_ids` |
| 4 | `POST /detect/ai-traces` | LLM only (실패 시 오류) | `detections[]` (`level`: GREEN/YELLOW/RED) |
| 5 | `POST /review/feedback` | LLM only (실패 시 오류) | `strengths`, `weaknesses`, 문단별 `improvement` |
| 6 | `POST /generate/interview-questions` | LLM only (실패 시 오류) | `questions[]` (`category`, `question`) |
| 7 | `POST /compare/keywords` | LLM only (실패 시 오류) | 매칭/누락 키워드 통계 |
...
| 7 | (관리자) `/admin` AI 로그 | `ai_usage_logs` 기록 + 모델명 표시 |

**Admin LLM 라우트 미설정 시:** TC-AI-03~05 중 LLM 호출 단계 FAIL 예상 — `/admin/llm-settings`와 `generate/resume`부터 확인

**자동화 (CI):** `e2e/tests/ai-flow.spec.ts` — `deploy-smoke-e2e-ai.sh` (master push 4단계). 외부 OpenAI 호환 API를 호출합니다. RAG 임베딩은 Admin EMBEDDING 라우트(실패 시 hash fallback).

---

## TC-10 — 공개 HTTPS (nginx)

| # | 검증 | 기대 |
|---|------|------|
| 1 | `https://resume.ggury.com/` · `/admin/` | 200 |
| 2 | `POST /api/v1/ai/generate` (장시간) | nginx 504 없이 완료 (`proxy_read_timeout` 300s) |
