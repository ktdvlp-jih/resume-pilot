...
AI       : Python 3.12 · FastAPI · Uvicorn · pip (requirements.txt)
           · Pydantic v2 · httpx
           · LangChain `ChatOpenAI` — 외부 OpenAI 호환 API 호출 (모델은 Admin `/admin/llm-settings`)
           · OpenAI SDK — prompt-service `/prompts/test` 폴백, rag-service 임베딩 폴백
           · 서버에 LLM 가중치를 올리지 않음 (로컬 Hugging Face 호스팅 아님)
...
Infra    : Docker · Docker Compose (prod 5컨테이너) · Eclipse Temurin 21 · python:3.12-slim
           ...
           · 호스트 nginx (resume.ggury.com → :9180, TLS)
...
```
Browser ──> nginx :443 (resume.ggury.com) ──> app (9180): SPA / + /admin + /api/v1
                ├──> resume-ai (internal)
                │         ├── prompt-service
                │         └── rag-service
                └──> PostgreSQL
...
4. LLM 호출 실패 시 규칙 기반 폴백 없이 오류를 반환한다 (API 키·라우트 필수)
5. LLM은 외부 API 호출 (Admin `/admin/llm-settings`). 서버에 모델 가중치를 올리지 않는다
