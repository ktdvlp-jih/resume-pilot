-- 자소서 생성(RESUME_GENERATION) 프롬프트 v17
-- 문항별 목표 줄 수(section_target_lines) · 문항 다시 쓰기 사용자 지시(user_instruction)
-- 프로필은 비어 있어도 됨. RAG 경험·공고가 본문 근거.
...
VALUES
(
    'b0000017-0001-0001-0001-000000000001',
...
4. section_target_lines의 문항별 목표 줄 수에 가깝게 쓰되, 사실 부족 시 짧게.
...
## 문항별 목표 줄 수 (한 줄 ≈ 40자 · 사실 부족 시 짧게)
{{section_target_lines}}

## 사용자 추가 지시 (선택 · RAG 밖 사실 금지)
{{user_instruction}}

위 RAG 경험과 공고만 사용해 자기소개서 초안 본문을 작성하세요.
목표 줄 수에 가깝게 쓰되, 사실을 지어내 분량을 채우지 마세요.$$,
    '["experiences","job_analysis","writing_style","rewrite_level","section_titles","section_target_lines","user_instruction"]'::jsonb,
    true
)
...
WHERE prompt_template_id = 'a0000001-0001-0001-0001-000000000001'
  AND id <> 'b0000017-0001-0001-0001-000000000001';

UPDATE prompt_versions SET is_active = true
WHERE id = 'b0000017-0001-0001-0001-000000000001';

UPDATE prompt_templates
SET active_version_id = 'b0000017-0001-0001-0001-000000000001', updated_at = NOW()
