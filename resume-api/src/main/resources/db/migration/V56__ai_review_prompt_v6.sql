-- AI 첨삭(AI_REVIEW) 프롬프트 v6
-- v5 문체 지적 유지. Pain Point 미연결·빈 글·판단 부재를 weaknesses/improvement에 반영.
-- reviews/scores JSON 스키마 불변.
...
VALUES (
    'b0000016-0001-0001-0001-000000000004',
...
- star_application: STAR·제약→판단→결과 흐름
...
[Task · 작업]
문단별 강점·약점·적합도·구체성·설득력·STAR를 평가하세요.
...
[Output · 출력]
JSON 객체 하나: reviews[], scores{company_fit,style_retention,star_application,experience_utilization}.$$,
    $$[자기소개서]
{{content}}
...
WHERE prompt_template_id = 'a0000001-0001-0001-0001-000000000004'
  AND id <> 'b0000016-0001-0001-0001-000000000004';

UPDATE prompt_versions SET is_active = true
WHERE id = 'b0000016-0001-0001-0001-000000000004';

UPDATE prompt_templates
SET active_version_id = 'b0000016-0001-0001-0001-000000000004', updated_at = NOW()
