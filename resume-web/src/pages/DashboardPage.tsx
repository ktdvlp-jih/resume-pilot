  ...
  return (
    <div className="max-w-6xl mx-auto">
      ...
      <div className="flex justify-between items-center mb-6 pt-2 border-t border-zinc-200/60 dark:border-zinc-800">
        ...
        <Link to="/workspace" className="ui-btn-primary">
          ...
          {resumes.map((r) => (
            ...
            >
              ...
              <div className="flex gap-3 mt-4 text-sm">
                <Link to={`/resumes/${r.id}/versions`} className="ui-link">{t('dashboard.versions')}</Link>
                <button type="button" onClick={() => deleteMutation.mutate(r.id)} className="ui-link-danger">{t('common.delete')}</button>
              </div>
            </div>
          ))}
        </div>
