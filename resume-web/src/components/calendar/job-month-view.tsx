...
type JobMonthViewProps = MonthCursorProps &
  Omit<MonthGridProps, 'className'> & {
    className?: string;
  };
...
function useMonthNavigation(cursor: YearMonth, onSelect: (key: string) => void) {
  ...
  return { goMonth, goToday: () => onSelect(today) };
}

export function JobMonthNav({ cursor, onSelect, locale, className }: MonthCursorProps) {
  const { t } = useTranslation();
  const { goMonth, goToday } = useMonthNavigation(cursor, onSelect);
...
export function JobMonthView({
  ...
  return (
    <div className={cn('flex flex-col gap-3', className)}>
      <JobMonthNav cursor={cursor} onSelect={onSelect} locale={locale} />
      <JobMonthGrid
        cursor={cursor}
